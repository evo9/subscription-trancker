...
| Tool | When to Use |
|------|-------------|
| `search-docs` | Laravel deployment docs |
...
| Frontend Build | Next.js + npm (apps/web) |
...
docker/nginx/                    # nginx + php-fpm config
```
...
- Cache npm dependencies (`node_modules/`) (apps/web)
- Cache Docker layers
...
- Run PHPStan and Pint checks in parallel
...
## php-fpm / nginx Tuning

### Pool Management
- Set `pm.max_children` based on available memory
- Choose the right `pm` mode (`dynamic` vs `static`) for the workload
- Set `pm.max_requests` to recycle workers and avoid leaks
- Tune `request_terminate_timeout` for long requests

### Performance
- Cache config/routes (`config:cache`, `route:cache`) in production
...
| CI/CD pipelines | Controllers | Query optimization |
| Deployment workflows | frontend (apps/web) | Migrations content |
