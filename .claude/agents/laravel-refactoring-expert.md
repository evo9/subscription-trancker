...
| `php-pro` | PHP 8.3+ strict typing, modern features |
...
| **Action classes** | `app/Actions/{Domain}/*` | Handle requests (plain `handle()`) |
| **Business Actions** | `app/Actions/{Domain}/*` | Reusable business logic |
...
| **API Resources** | `app/Http/Resources/` | JSON response shaping |

> **Thin controllers only (logic in Action classes); no Repositories, no `app/Domain/` directory.**
...
3. Run existing tests to establish baseline: `docker compose exec api php artisan test --filter=TargetClass`
4. Identify code smells (complexity metrics, duplication, coupling)
5. Check cognitive complexity limits (function: 8, class: 85)

### Phase 2: Strategy
Develop a plan that:
- Aligns with the Action-class pattern (thin controllers → Actions; no Repository layer)
- Uses PHP 8.3 features (readonly, enums, match, named arguments)
...
// AFTER: Thin controller + Business Action
class PostController extends Controller
{
    public function store(StorePostRequest $request, CreatePost $createPost): PostResource
    {
...
1. Run existing tests: `docker compose exec api php artisan test --compact`
2. Run code style: `docker compose exec api ./vendor/bin/pint --dirty`
3. Run static analysis: `docker compose exec api ./vendor/bin/phpstan analyse`
...
docker compose exec api php artisan test --filter=TargetClass
...
| Complexity reduction | frontend (apps/web) | Index strategy |
| N+1 query fixes | Form handling | Migration design |
| Extract method/class | API endpoints | Query performance |
| Pattern alignment | API Resources | Database tuning |
