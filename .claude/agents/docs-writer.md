...
| Server | php-fpm behind nginx |
| Styling | Tailwind CSS |
| Package Manager | Composer (api) + npm (web) |
| Architecture | Plain Action classes + thin controllers (lorisleiva/laravel-actions optional) |
...
- PHP 8.3+ syntax (readonly properties, enums, match expressions, type hints)
- Laravel 13 conventions (thin controllers delegating to Action classes; `query()` method; `getKey()`)
- Pest 4 BDD syntax (`it()`, `describe()`, `expect()`)
- `docker compose exec api` for all commands
...
- Prerequisites (PHP 8.3+, Node.js, PostgreSQL 17, Redis, Docker)
...
- [ ] All code examples use PHP 8.3+ / Laravel 13 syntax
- [ ] All commands use `docker compose exec api` prefix
