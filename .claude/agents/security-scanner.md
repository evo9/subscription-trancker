...
- **Type hints**: PHP 8.3 strict types throughout
...
- [ ] XSS prevention (API returns JSON; the React client (apps/web) escapes output)
...
docker compose exec api git log --all -p -- '*.env' '*.key' | head -50

# List routes without middleware
docker compose exec api php artisan route:list --columns=method,uri,middleware

# Check PHP configuration
docker compose exec api php -i | grep -E "(expose_php|display_errors|allow_url)"
...
| Input validation review | frontend (apps/web) | Firewall rules |
