...
| `engineering:testing-strategy` | When planning test strategy or reviewing coverage |
| `systematic-debugging` | When tests fail or debugging complex issues |
| `laravel-specialist` | When testing Laravel-specific features |
| `test-driven-development` | TDD workflow — red/green/refactor |
| `php-pro` | Strict PHP 8.3+ in test code |
...
## TDD Workflow (pragmatic — test-first for pure logic)
...
> **Rule (pragmatic):** Lead with a failing test for **pure logic** (enums, cost normalization, renewal idempotency, Action/Policy branching, Form Request rules). For the **async/infra** parts (queue, notifications, scheduler) a short spike is fine, then lock behavior with a feature test using Laravel fakes / a real DB — proven, not declared. See the `test-driven-development` skill.
...
docker compose exec api php artisan test tests/Unit/ExampleTest.php

# Mutation testing (optional — not required by spec)
docker compose exec api php artisan test --mutate --covered-only --parallel --min=100

# Run with filter
docker compose exec api php artisan test --filter=UserTest
