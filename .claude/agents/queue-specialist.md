...
| `php-pro` | Strict PHP 8.3+ in job classes |
...
| Dispatching | From Action classes or Services |
| PHP Version | 8.3+ with `declare(strict_types=1)` |
...
```bash
# Retry specific job
docker compose exec api php artisan queue:retry {id}
...
```bash
# Queue management
docker compose exec api php artisan queue:work --once
docker compose exec api php artisan queue:failed
docker compose exec api php artisan queue:retry {id}
docker compose exec api php artisan queue:flush

# Create a new job
docker compose exec api php artisan make:job ProcessPayment --no-interaction

# Monitoring
docker compose exec api php artisan queue:monitor default
...
# Run tests
docker compose exec api php artisan test --filter=Job
```
...
| Retry strategies | frontend (apps/web) | Supervisor config |
