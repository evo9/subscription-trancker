...
| **419** | CSRF token expired, session/token mismatch (Sanctum) |
...
### Frontend Boundary Issues (apps/web)
- **Stale Data**: Client shows old data → check TanStack Query cache invalidation
- **Validation Errors Not Showing**: Check the 422 response shape vs. RHF/Zod error mapping
- **CORS/Auth Failures**: Check Sanctum config, allowed origins, and token handling
...
docker compose exec api grep -n "ErrorClassName" storage/logs/laravel.log
...
docker compose exec api php artisan test --filter=BugReproductionTest
...
| Reproduction strategy | frontend (apps/web) | TDD workflows |
