...
| PHP Version | 8.3+ |
| Node/Package Manager | Node.js + npm (apps/web) |
...
- Prettier (frontend formatting, apps/web)
...
- **npm**: Cache `node_modules/` keyed by `package-lock.json` hash (apps/web)
...
docker compose exec api php artisan test --mutate --covered-only --parallel --min=100
...
> The API service is named `api`; the web service is named `web`.
...
- Cache Composer, npm, and Docker layers separately
...
- Node.js with npm for frontend builds (apps/web)
...
| Docker image builds | php-fpm/nginx tuning | Controllers |
| CI caching strategy | Redis config | Business logic |
| Pipeline optimization | Queue workers | frontend (apps/web) |
...
- [ ] Caching configured for Composer, npm, Docker layers
...
- [ ] Docker commands use `docker compose exec api` prefix
...
- **Always use `docker compose exec api` prefix** (NOT `api`)
